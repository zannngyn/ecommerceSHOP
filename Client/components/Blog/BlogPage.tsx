      ...
      <Dialog open={dialog} onClose={() => setdialog(false)} className="relative z-50">
        <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
          <DialogPanel className="max-w-5xl space-y-4 overflow-y-auto max-h-screen border bg-white p-8 rounded-xl">
            ...
            <p className='font-medium text-end'>Đăng bởi <span className='rounded-xl px-2 py-2 bg-salmon text-white'>{selectedData.current.author}</span></p>
          ...
          {data.current.map((each, index) => (
            <div key={index} className='flex flex-col gap-5 min-w-[300px] snap-center'>
              ...
              <div className='flex flex-col max-w-[300px]'>
                ...
                <p className='text-silver tracking-wider'>Đăng bởi <span className='text-davysilver'>{each.author}</span> / {formatDate(each.published_date)}</p>
              </div>
            </div>
          ))}
        </div>
