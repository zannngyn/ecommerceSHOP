    ...
    return (
        <section className="bg-white py-8 h-screen w-screen overflow-x-hidden antialiased relative dark:bg-gray-900 md:py-6">
            <Dialog open={dialogType === 'addressRequired'} onClose={() => setdialogType('addressRequired')} className="relative z-50">
                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                    <DialogPanel className="max-w-lg space-y-4 border bg-white p-12 rounded-xl text-center drop-shadow-custom-xl">
                        <DialogTitle className="font-bold">Yêu cầu địa chỉ</DialogTitle>
                        <Description>Vui lòng thêm địa chỉ để tiếp tục thanh toán</Description>
                        <div className="flex justify-center gap-4">
                            <button className='border-[1.5px] hover:bg-black transition-colors duration-300 hover:text-white py-2 px-6 rounded-xl' onClick={() => { router.push('/account-settings') }}>Đi đến cài đặt tài khoản</button>
                        </div>
                    </DialogPanel>
                </div>
            </Dialog>
            <Dialog open={dialogType === 'defaultAddressRequired'} onClose={() => setdialogType('defaultAddressRequired')} className="relative z-50">
                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                    <DialogPanel className="max-w-lg space-y-4 border bg-white p-12 rounded-xl text-center drop-shadow-custom-xl">
                        <DialogTitle className="font-bold">Yêu cầu địa chỉ mặc định</DialogTitle>
                        <Description>Vui lòng thêm địa chỉ mặc định hoặc đặt địa chỉ hiện có làm mặc định để tiếp tục thanh toán</Description>
                        <div className="flex justify-center gap-4">
                            <button className='border-[1.5px] hover:bg-black transition-colors duration-300 hover:text-white py-2 px-6 rounded-xl' onClick={() => { router.push('/account-settings') }}>Đi đến cài đặt tài khoản</button>
                        </div>
                    </DialogPanel>
                ...
                <div className="mt-6 sm:mt-8 lg:flex lg:items-start lg:gap-12 xl:gap-16">
                    <form action="/" id='informational-form' onSubmit={createOrder} className="min-w-0 flex-1 space-y-8">
                        <div className="space-y-4">
                            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Thông tin giao hàng</h2>
                            ...
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    <div className="mb-2 flex items-center gap-2">
                                        <label htmlFor="select-country-input-3" className="block text-sm font-medium text-gray-900 dark:text-white"> Quốc gia* </label>
                                    </div>
                                ...
                                <div>
                                    <div className="mb-2 flex items-center gap-2">
                                        <label htmlFor="select-city-input-3" className="block text-sm font-medium text-gray-900 dark:text-white"> Thành phố* </label>
                                    </div>
                                ...
                                <div>
                                    <label htmlFor="phone-input-3" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white"> Số điện thoại* </label>
                                ...
                                <div>
                                    <label htmlFor="pincode" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white"> Mã bưu chính </label>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 ps-4 dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-start">
                                        ...
                                        <div className="ms-4 text-sm">
                                            <label htmlFor="online-payment" className="font-medium leading-none text-gray-900 dark:text-white"> Thanh toán trực tuyến </label>
                                            <p id="online-payment-text" className="mt-1 text-xs font-normal text-gray-500 dark:text-gray-400">Thanh toán bằng ngân hàng/thẻ</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 ps-4 dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-start">
                                        ...
                                        <div className="ms-4 text-sm">
                                            ...
                                            <p id="pay-on-delivery-text" className="mt-1 text-xs font-normal text-gray-500 dark:text-gray-400">+15$ phí xử lý thanh toán</p>
                                        </div>
                                    </div>
                                </div>
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Delivery Methods</h3> */}
                ...
                <div className="flex items-start">
                    ...
                    <p id="express-text" className="mt-1 text-xs font-normal text-gray-500 dark:text-gray-400">Get it today</p>
                            ...
                            <div className="flex border-[1px] min-h-[150px] rounded-xl px-6 py-6 items-center mb-4">
                                ...
                                <div className='flex flex-col gap-5 sm:gap-0 sm:flex-row justify-between w-full items-center'>
                                    <div className={`flex flex-col ${(data.sizename != null || data.colorname != null) ? 'gap-1' : 'gap-5'}`}>
                                        ...
                                        {data.sizename != null && <p className='text-sm font-medium'>Kích thước: <span className='font-semibold'>{data.sizename}</span></p>}
                                        {data.colorname != null && <p className='text-sm font-medium'>Màu sắc: <span className='font-semibold'>{data.colorname}</span></p>}
                                        <p className='text-sm text-silver'>Số lượng: <span className='text-black font-semibold'>{1}</span></p>
                                    </div>
                                    <div>
                        ...
                        <div>
                            <label htmlFor="voucher" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Nhập mã quà tặng, voucher hoặc mã khuyến mãi</label>
                            <div className="flex max-w-md items-center gap-4">
                                ...
                                <button type="button" className="flex items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Áp dụng</button>
                            </div>
                        </div>
                    </form>
                    ...
                    <div className="mt-6 w-full space-y-6 sm:mt-8 lg:mt-0 lg:max-w-xs xl:max-w-md">
                        ...
                        <div className="space-y-3">
                            {!onlinePayment && <button form='informational-form' disabled={!loggedIn} type="submit" className="flex w-full items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4  focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Đặt hàng</button>}
                        </div>
