...
const Details = () => {
    ...
    return (
        <div className='flex w-full flex-wrap justify-center mt-5 gap-8'>
            <div>
                <p className='tracking-base text-xl font-semibold text-eblack border-b-[1px] pb-3 border-b-gray-200'>Lời chứng thực</p>
            ...
            <div className='w-[640px] h-[450px] relative rounded-xl justify-center items-center'>
                ...
                <a href={currentEvent.eventLink} className='w-[50%] h-[60%] left-0 right-0 top-0 bottom-0 m-auto absolute bg-transparent rounded-md flex flex-col items-center justify-center gap-2'>
                    <p className={`text-white bg-black p-2 rounded-md font-bold text-base text-center tracking-wider ${currentEvent.isDiscount ? 'block' : 'hidden'}`}>{currentEvent.discount}% GIẢM</p>
                    ...
                    <p className='text-lg text-silver'>Giá từ ${currentEvent.starting}</p>
                    <button className='font-bold text-lg text-silver'>MUA NGAY</button>
                </a>
            </div>
            <div>
                <p className='tracking-base text-xl font-semibold text-eblack border-b-[1px] pb-3 border-b-gray-200 tracking-wide'>Dịch vụ của chúng tôi</p>
