            ...
            <Dialog open={otpPopup} onClose={() => setotpPopup(false)} className="relative z-40">
                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                    <DialogPanel className="max-w-lg space-y-4 border bg-white p-12 rounded-xl text-center font-medium">
                        <DialogTitle className="font-bold">OTP đã được gửi</DialogTitle>
                        <Description>Mã OTP đã được gửi đến email của bạn, vui lòng kiểm tra hộp thư.</Description>
                        ...
                        <div>
                            <Dialog open={incorrectOTP} onClose={() => setIncorrectOTP(false)} className="relative z-50">
                                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                                    <DialogPanel className="max-w-lg space-y-4 border bg-white p-12 rounded-xl text-center drop-shadow-custom-xl">
                                        <DialogTitle className="font-bold">Sai OTP</DialogTitle>
                            ...
                            <Dialog open={expiredOTP} onClose={() => setExpiredOTP(false)} className="relative z-50">
                                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                                    <DialogPanel className="max-w-lg space-y-4 border bg-white p-12 rounded-xl text-center drop-shadow-custom-xl">
                                        <DialogTitle className="font-bold">OTP đã hết hạn</DialogTitle>
                                        <Description>Mã OTP đã hết hạn. Vui lòng thử lại.</Description>
                                        <div className="flex justify-center gap-4">
                            ...
                            <Dialog open={success} onClose={() => setSuccess(false)} className="relative z-50">
                                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                                    <DialogPanel className="max-w-lg space-y-4 border bg-white p-12 rounded-xl text-center drop-shadow-custom-xl">
                                        ...
                                        <Description>Mật khẩu đã được thay đổi thành công.</Description>
                                        <p>Nhấn vào Đăng nhập để vào tài khoản</p>
                                        <div className="flex justify-center gap-4">
                            ...
                            <Dialog open={resent} onClose={() => setResent(false)} className="relative z-50">
                                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                                    <DialogPanel className="max-w-lg space-y-4 border bg-white p-12 rounded-xl text-center drop-shadow-custom-xl">
                                        ...
                                        <Description>Mã OTP đã được gửi lại. Vui lòng kiểm tra hộp thư.</Description>
                                        <div className="flex justify-center gap-4">
                        ...
                        <div className="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                ...
                                <form onSubmit={(e) => resetPass(e)} method='post' className="space-y-4 md:space-y-6 flex flex-col gap-4 lg:gap-0" action="/">
                                    ...
                                    <button type="submit" className="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Lấy OTP & Đặt lại mật khẩu</button>
                                </form>
