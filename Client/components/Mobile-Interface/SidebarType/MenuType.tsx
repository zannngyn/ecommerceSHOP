    ...
    return (
        <>
            <div className='flex w-[90%] items-center justify-between border-b-[1px] pb-4 -left-96'>
                <p className='text-salmon font-bold text-xl tracking-wide'>Thực đơn</p>
                ...
                {navBtns.map((each, index) => each.name != 'Categories' &&
                    <div key={index}>
                        ...
                        {each.isExtendable && <div
                            className={`transition-[max-height] duration-[400ms] ease-linear overflow-hidden ${collapsedIndex === index ? 'max-h-[160px]' : 'max-h-0'
                                }`}>
                            {(
            ...
            <div className='w-[90%] border-t-[1px]'>
                ...
                {loggedIn && <button onClick={signOut} className='flex justify-between mt-1 items-center text-gray-700  pb-4 hover:text-black'>Đăng xuất</button>}
            </div>
        </>
    )
}
