    ...
    return (
        <div className='flex flex-col gap-10'>
            ...
            <section className='w-[95%] mx-auto flex flex-col gap-2'>
                <p className='font-semibold text-3xl'>Lịch sử đơn hàng</p>
                <p className='text-silver text-sm'>Kiểm tra trạng thái đơn hàng gần đây, quản lý trả hàng và khám phá các sản phẩm tương tự.</p>
            </section>
                        ...
                        <div className='flex gap-8 justify-between items-center  border-b-[1px] pb-4'>
                            <div className='flex gap-10 items-center'>
                                <div className='flex flex-col gap-1'>
                                    <p className='font-medium text-sm'>Số đơn hàng</p>
                                ...
                                <div className='hidden md:flex-col gap-1 md:flex'>
                                    <p className='font-medium text-sm'>Ngày đặt</p>
                                    <p className='text-silver font-medium text-sm'>{formatDate(order.createdat)}</p>
                                </div>
                                <div className='flex flex-col gap-1'>
                                    <p className='font-medium text-sm'>Tổng số tiền</p>
                            ...
                            <div className='gap-5 hidden md:flex'>
                                <button onClick={() => router.push(`/order-detail/${order.orderid}`)} className='border-[1px] border-gray-300 py-2 px-3 rounded-lg bg-white text-davysilver font-medium text-sm transition-colors duration-150 hover:bg-btnpurple hover:text-white'>Xem đơn hàng</button>
                                <button className='border-[1px] border-gray-300 py-2 px-3 rounded-lg bg-white text-davysilver font-medium text-sm transition-colors duration-150 hover:bg-btnpurple hover:text-white'>Xem hóa đơn</button>
                            </div>
                        </div>
                        ...
                        <div className='flex justify-between flex-col sm:flex-row items-end pt-2'>
                            {order.orderstatus === 'Delivered' ?
                                <div className='flex items-center gap-2 mx-auto sm:mx-0'>
                                    ...
                                    <p className='text-sm font-medium'>Đã giao vào {formatDate(order.deliveredat)}</p>
                                </div>
                                :
                                <div className='flex items-center gap-2 mx-auto sm:mx-0'>
                                    ...
                                    <p className='text-sm font-medium'>Dự kiến giao vào {formatDate(order.deliveredat)}</p>
                                </div>
                            }
                            <div className='mt-3 mb-3 h-[1px] bg-gray-200 w-full flex sm:hidden'></div>
                            <div className='w-full justify-evenly flex sm:w-auto'>
                                <Link href={`/product/${order.productid}`}><button className='sm:border-r-[1px] sm:px-4 text-sm font-medium text-btnpurple'>Xem sản phẩm</button></Link>
                                <div className='h-[20px] border-r-[1px] bg-gray-200 w-[1px] flex sm:hidden'></div>
                                <button className='px-4 text-sm font-medium text-btnpurple'>Mua lại</button>
                            </div>
                        </div>
                    </div>
                </section>)}
