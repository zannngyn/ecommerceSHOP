  ...
  return (
    <div>
      ...
      {(dataChecked.current && found.current && loggedIn.current) && <div className="2xl:w-4/6 mx-auto p-4 bg-white rounded-lg mt-10 mb-10">
        <h2 className="text-4xl font-medium mb-12">Chi tiết đơn hàng của bạn</h2>
        <div className="text-lg font-medium mb-4">Đơn hàng <span className='font-semibold'>#{data.order_code}{data.orderid}</span></div>
        <div className="text-gray-700 mb-8 text-lg">Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi!</div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          <div className="border p-8 rounded-xl gap-6 flex flex-col">
            <h3 className="font-semibold text-xl mb-2">Thông tin đơn hàng</h3>
            <div className='grid grid-cols-2'>
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Ngày đặt hàng</p>
                <p className='font-medium'>{formatDate(data.createdat)}</p>
              </div>
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Ngày giao hàng</p>
                <p className='font-medium'>{formatDate(data.deliveredat)}</p>
              </div>
            </div>
            <div className='grid grid-cols-2'>
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Trạng thái</p>
                <p className='font-medium'>{data.orderstatus}</p>
              </div>
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Tình trạng thanh toán</p>
                <p className='font-medium'>{data.paymentstatus}</p>
              </div>
            </div>
            <div className='flex flex-col gap-2'>
              <p className='text-silver'>Phương thức thanh toán</p>
              <p className='font-medium'>{data.paymentmethod}</p>
            </div>
          </div>
          <div className="border p-8 rounded-xl gap-6 flex flex-col">
            <h3 className="font-semibold text-xl mb-2">Khách hàng</h3>
            <div className='grid grid-cols-2'>
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Tên:</p>
            ...
            <div className='flex gap-8'>
              ...
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Số điện thoại</p>
                <p className='font-medium'>+84 {data.mobile_number}</p>
              </div>
            </div>
          </div>
          <div className="border p-8 rounded-xl gap-6 flex flex-col">
            <h3 className="font-semibold text-xl mb-2">Địa chỉ</h3>
            <div>
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Địa chỉ giao hàng</p>
            ...
            <div>
              <div className='flex flex-col gap-2'>
                <p className='text-silver'>Địa chỉ thanh toán</p>
          ...
          <div className="flex border-[1px] min-h-[150px] rounded-xl px-6 py-6 items-center mb-4">
            ...
            <div className='flex flex-col gap-5 sm:gap-0 sm:flex-row justify-between w-full items-center'>
              <div className={`flex flex-col ${(data.sizename != null || data.colorname != null) ? 'gap-1' : 'gap-5'}`}>
                ...
                <p className='text-lg text-silver'>Số lượng: <span className='text-black font-semibold'>{data.quantity}</span></p>
              </div>
