        ...
        <>
            ...
            <div>
                <Dialog open={dialogType === 'create'} onClose={() => setdialogType(null)} className="relative z-50">
                    <div className="fixed inset-0 flex w-screen items-center justify-center p-4 drop-shadow-custom-xl">
                        <DialogPanel className="max-w-lg space-y-4 border bg-white p-8 rounded-xl text-center">
                            ...
                            <Description>Chia sẻ đánh giá của bạn</Description>
                            <form onSubmit={createForm} className='flex flex-col gap-2'>
                                <div className='flex flex-col items-center'>
                                    <label>Số sao</label>
                                    {starWarning && <p className='text-red-500'>Vui lòng đánh giá ít nhất 1 sao để tiếp tục</p>}
                                    <ReactStars
                                        count={5}
                                        ...
                                        color2={'#ffd700'} />
                                    <label>Tiêu đề</label>
                                    <input placeholder='Tiêu đề đánh giá (tối đa 50 ký tự)' required id='title' type='text' minLength={2} maxLength={50} className='border-[1px] w-[300px] rounded-md py-1 mx-auto' />
                                    <label>Nội dung</label>
                                    <textarea placeholder='Nội dung đánh giá (tối đa 500 ký tự)' required id="description" minLength={2} maxLength={1500} rows={4} cols={40} className='border-[1px] rounded-md py-1 mx-auto'>
                                    </textarea>
                                </div>
                ...
                <Dialog open={dialogType === 'edit'} onClose={() => setdialogType(null)} className="relative z-50">
                    <div className="fixed inset-0 flex w-screen items-center justify-center p-4 drop-shadow-custom-xl">
                        <DialogPanel className="max-w-lg space-y-4 border bg-white p-8 rounded-xl text-center">
                            <DialogTitle className="font-bold">Chỉnh sửa đánh giá</DialogTitle>
                            <Description>Chia sẻ đánh giá của bạn</Description>
                            <form onSubmit={editForm} className='flex flex-col gap-2'>
                                <div className='flex flex-col items-center'>
                                    ...
                                    <ReactStars
                                        ...
                                        color2={'#ffd700'} />
                                    <label>Tiêu đề</label>
                                    <input defaultValue={selectedReview != null ? selectedReview.title : ''} placeholder='Tiêu đề đánh giá (tối đa 50 ký tự)' required id='title' type='text' minLength={2} maxLength={50} className='border-[1px] w-[300px] rounded-md py-1 mx-auto' />
                                    <label>Nội dung</label>
                                    <textarea defaultValue={selectedReview != null ? selectedReview.comment : ''} placeholder='Nội dung đánh giá (tối đa 500 ký tự)' required id="description" minLength={2} maxLength={1500} rows={4} cols={40} className='border-[1px] rounded-md py-1 mx-auto'>
                                    </textarea>
                                </div>
                                <div className="flex justify-center gap-4">
                                    <button type='button' className='border-[1.5px] hover:bg-black transition-colors duration-300 hover:text-white py-2 px-6 rounded-xl' onClick={() => setdialogType(null)}>Hủy</button>
                ...
                <Dialog open={dialogType === 'delete'} onClose={() => setdialogType(null)} className="relative z-50">
                    <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                        <DialogPanel className="max-w-lg space-y-4 border  p-6 rounded-xl text-center drop-shadow-custom-xl bg-red-400 text-white">
                            ...
                            <Description>Bạn có chắc muốn xóa đánh giá không?</Description>
                            <div className="flex justify-center gap-4">
                                ...
                                <button className='border-[1.5px] hover:bg-white transition-colors duration-300 hover:text-black py-2 px-6 rounded-xl' onClick={deleteForm}>Xóa</button>
                            </div>
                        </DialogPanel>
                    </div>
                </Dialog>
