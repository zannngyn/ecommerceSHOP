    ...
    >
      ...
      <div className='pl-4 pr-4 flex flex-col gap-2'>
        ...
        <div className='flex items-center gap-2'>
          ...
          {product.reviewCount > 0 && <p className='text-silver'>{product.reviewCount} đánh giá</p>}
        </div>
        <div className='flex mb-5 items-center gap-4'>
          <p className='font-bold text-[18px]'>{parseInt(product.discount).toLocaleString('vi-VN')}₫</p>
          <p className='line-through'>{parseInt(product.price).toLocaleString('vi-VN')}₫</p>
        </div>
      </div>
    </div>
  );
