...
router.post('/product/create/size', async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Thêm kích thước thành công' });
    } catch (error) {
        res.status(500).json({ message: 'Lỗi máy chủ' });
    }
});
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            if (response.rows.length > 0) {
                return res.status(205).json({ message: 'Đánh giá đã tồn tại' });
            }
        } catch (error) {
        ...
        try {
            ...
            if (response.rows.length === 0) {
                return res.status(210).json({ message: 'Đơn hàng không tồn tại' });
            }
        } catch (error) {
            return res.status(500).json({ error: 'Lỗi máy chủ' });
        }
...
router.patch('/review/edit', editReviewSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            const response = await client.query(checkQuery, checkValue);
            if (response.rows.length === 0) {
                return res.status(205).json({ message: 'Đánh giá không tồn tại' });
            }
        } catch (error) {
            return res.status(500).json({ error: 'Lỗi máy chủ' });
        }
