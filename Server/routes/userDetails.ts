    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Không tìm thấy địa chỉ cho người dùng này' });
            }
...
router.post('/user/cart-items', userIDSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Không tìm thấy mục giỏ hàng cho người dùng này' });
            }
            res.status(200).json(result.rows);
        } catch (error) {
            res.status(500).json({ error: 'Lỗi máy chủ' });
...
router.post('/user/wishlist-items', async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Không tìm thấy mục yêu thích cho người dùng này' });
            }
            res.status(200).json(result.rows);
        } catch (error) {
            res.status(500).json({ error: 'Lỗi máy chủ' });
...
router.post('/user/coupons', userIDSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            const result = await client.query(query, values);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Không tìm thấy phiếu giảm giá cho người dùng này' });
            }
            res.status(200).json(
                { data: result.rows }
            );
...
router.post('/user/insert/cartitem', cartItemSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            res.status(200).json({ message: 'Thêm hoặc cập nhật mục giỏ hàng thành công' });
        } catch (error) {
            res.status(500).json({ message: 'Lỗi máy chủ' });
        }
    } else {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            if (checkResult.rows.length > 0) {
                ...
                res.status(200).json({ message: 'Mục đã tồn tại trong danh sách yêu thích' });
            } else {
                // Item does not exist, insert a new row
                await client.query(insertQuery, insertValues);
                res.status(200).json({ message: 'Thêm mục thành công' });
            }
        } catch (error) {
            res.status(500).json({ message: 'Lỗi máy chủ' });
        }
    } else res.status(500).json({ message: 'Lỗi máy chủ' });
});
