    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                ...
                return res.status(205).json({ error: 'Email không tồn tại' });
            }
        ...
        console.log(result);
        res.status(500).json({ message: 'Lỗi xác thực dữ liệu' });
    }
});
router.post('/user/reset-password', EmailPasswordOtpSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            const result = await client.query(query, values);
            if (result.rows.length === 0) {
                return res.status(205).json({ message: 'OTP không chính xác' });
            }
            ...
            if (diffMinutes >= 5) {
                return res.status(210).json({ message: 'OTP đã hết hạn' });
            }
        } catch (error) {
            return res.status(500).json({ error: 'Lỗi máy chủ' });
        }
