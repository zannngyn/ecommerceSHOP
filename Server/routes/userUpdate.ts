...
router.put('/user', userUpdateSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            if (updates.length === 0) {
                return res.status(400).json({ error: 'Chưa cung cấp trường để cập nhật' });
            }
...
router.post('/user/insert/address', insertAddressSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            res.status(200).json({ message: 'Thêm địa chỉ thành công', addressid: addressID });
        } catch (error) {
            res.status(500).json({ message: 'Lỗi máy chủ' });
        }
    } else {
...
router.post('/user/cart-quantity', cartQtyUpdate, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        try {
            ...
            res.status(500).json({ error: 'Đã có lỗi khi cập nhật' });
        }
    } else {
        console.log(result);
        res.status(500).json({ error: 'Lỗi xác thực dữ liệu' });
    }
})
